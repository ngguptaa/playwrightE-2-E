...
test.describe("Course bookmark  Tests - NaviJapan", () => {
  ...
  test("Should login successfully with valid email and OTP, then verify signout flow", async ({
    ...
    //step 3: click Course icon
    const coursesBtn = page.locator('button:has(span:text("Courses"))');
    // Wait for button to be visible, enabled, and not blocked
    await coursesBtn.waitFor({ state: "visible", timeout: 150000 });
    ...
    // step 4: Find the course card by its name
    await page.waitForSelector(
      'h3:has-text("Graduate School of Information Science and Technology (IST)")',
      { timeout: 15000 }
    );
