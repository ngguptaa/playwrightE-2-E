...
test('Should login successfully with valid email and OTP, then verify signout flow', async ({ page }) => {
  ...
  //step 3: click Scholarship icon 
   const scholarshipsBtn = page.locator('button:has(span:text("Scholarships"))');
  ...
  // step 4: Find the Scholarship card by its name
   await page.waitForSelector('h3:has-text("MEXT (Monbukagakusho) Scholarships")', { timeout: 15000 });
