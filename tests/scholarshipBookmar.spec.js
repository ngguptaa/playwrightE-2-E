...
test.describe("Scholarship bookmark  Tests - NaviJapan", () => {
  ...
  test("Should login successfully with valid email and OTP, then verify signout flow", async ({
    page,
  }) => {
    ...
    //step 3: click Scholarship icon
    const scholarshipsBtn = page.locator(
      'button:has(span:text("Scholarships"))'
    );
    // Wait for button to be visible, enabled, and not blocked
    await scholarshipsBtn.waitFor({ state: "visible", timeout: 150000 });
    await page.waitForFunction(
      () => !document.body.classList.contains("driver-active")
    );
    const closeBtn = page.locator('button:has-text("Close")');
    if (await closeBtn.isVisible()) await closeBtn.click();
    await scholarshipsBtn.click();

    // step 4: Find the Scholarship card by its name
    await page.waitForSelector(
      'h3:has-text("MEXT (Monbukagakusho) Scholarships")',
      { timeout: 15000 }
    );
